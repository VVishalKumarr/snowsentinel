  ...
  return (
    <div className={`rounded-2xl border p-4 sm:p-5 ${colors.bg} ${colors.border}`}>
      <div className="mb-3 flex items-center gap-2.5">
        ...
        <h2 className="text-sm font-semibold tracking-wide text-slate-100">PREPAREDNESS ALERT</h2>
        <span className={`ml-auto rounded-full border px-2.5 py-0.5 text-[10px] font-semibold tracking-wide ${colors.bg} ${colors.border} ${colors.text}`}>
      ...
      <div className="mt-4 border-t border-white/10 pt-4">
        <h3 className="mb-2 text-[11px] font-semibold tracking-[0.1em] text-slate-400">
          RECOMMENDED ACTIONS
        </h3>
        <ul className="space-y-1.5">
          {scenario.recommendedActions.map((action) => (
            <li key={action} className="flex items-start gap-2 text-sm text-slate-300">
              <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-emerald-400" strokeWidth={2} />
              {action}
            </li>
          ))}
        </ul>
